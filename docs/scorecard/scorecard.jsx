// Open Data Eval — Metadata Scorecard v3.0
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:8 }}>
      <div style={{ position:"relative", width:78, height:78, borderRadius:"50%",
        ...
        <span style={{ fontSize:15, fontWeight:700, color:col, lineHeight:1, userSelect:"none" }}>
          {isNull ? "—" : display}
        </span>
        {isGreen && (
...
// ─── scale stat ───────────────────────────────────────────────
function ScaleStat({ value, label, sub }) {
  return (
    <div style={{ flex:1, textAlign:"center", padding:"12px 8px" }}>
      <div style={{ fontSize:20, fontWeight:700, lineHeight:1,
        color: value != null ? "var(--c-text-1)" : "var(--c-text-3)" }}>
        {value != null ? value : "—"}
      </div>
      <div style={{ fontSize:11, color:"var(--c-text-2)", marginTop:3 }}>{label}</div>
      {sub && <div style={{ fontSize:10, color:"var(--c-text-3)", marginTop:2 }}>{sub}</div>}
    </div>
  );
}
...
function CalChecklist({ d }) {
  ...
  function Icon({ v }) {
    const style = { fontSize:15, width:18, display:"inline-block", textAlign:"center", lineHeight:1 };
    if (v === null || v === undefined) return <span style={{ ...style, color:"var(--c-null)" }}>?</span>;
    return v
      ? <span style={{ ...style, color:"var(--c-green)" }}>✓</span>
      : <span style={{ ...style, color:"var(--c-red)"   }}>✗</span>;
  }
  return (
    <div>
...
function ScoreCard({ d, usecase, onUsecaseChange }) {
  const [open, setOpen] = useState({
    technical:true, downstream:true, scale:false,
    reliability:false, access:false, hardware:false,
  });
  const [modalOpen, setModalOpen] = useState(false);
  const toggle = key => setOpen(s => ({...s, [key]:!s[key]}));

  const shortEdge = d.resW && d.resH ? Math.min(d.resW, d.resH) : null;
  const dlLabel = d.dl === 1.0
  ...
  return (
    <div style={{ background:"var(--c-surface)", border:"1px solid var(--c-border)",
      borderRadius:12, maxWidth:560, width:"100%", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          <div style={{ flex:1, paddingRight:16 }}>
            ...
            <div style={{ fontSize:12, color:"var(--c-text-3)", marginTop:2 }}>
              ...
              {d.citations != null ? ` · ~${d.citations.toLocaleString()} citations` : ""}
            </div>
            {/* modality expand */}
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, flexShrink:0 }}>
            ...
            <span style={{ fontSize:9, color:"var(--c-text-3)" }}>fields filled</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:"0 22px" }}>
        ...
        <Section title="Scale & Diversity" open={open.scale} onToggle={() => toggle("scale")}>
          <div style={{ display:"flex", borderRadius:8, border:"1px solid var(--c-border)",
            overflow:"hidden", marginBottom:10 }}>
            <ScaleStat
              value={d.hrs ? Number(d.hrs).toLocaleString() : null}
              label="hours" />
            <div style={{ width:1, background:"var(--c-border)" }} />
            <ScaleStat
              value={d.envDivCount > 0 ? Math.round(d.envDivCount).toLocaleString() : null}
              label="environments" />
            <div style={{ width:1, background:"var(--c-border)" }} />
            <ScaleStat
              value={d.demoParts != null ? Math.round(d.demoParts).toLocaleString() : null}
              label="participants"
              sub={d.demoGeo != null ? `${d.demoGeo} countries` : null} />
          </div>
          {d.dlEffHPG != null && (
            <div style={{ display:"flex", alignItems:"center", gap:9 }}>
              <span style={{ fontSize:12, color:"var(--c-text-3)" }}>Download efficiency</span>
              <span style={{ fontSize:12, padding:"3px 11px", borderRadius:20, fontWeight:500,
...
function CompareTable({ usecase, sortBy, onSortChange, onRowClick }) {
  ...
  // 0-1 score cell, displayed as 0-100 integer
  const ScoreCell = ({ v }) => (
  ...
  // raw number cell (hours, envs, participants, citations)
  const NumCell = ({ v }) => (
    <td style={{ textAlign:"right", padding:"6px 6px", fontSize:12,
...
function App() {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px", fontFamily:"var(--font)" }}>
      ...
      <div style={{ marginBottom:22 }}>
        <div style={{ display:"flex", alignItems:"baseline", gap:10, flexWrap:"wrap" }}>
          ...
          <span style={{ fontSize:13, color:"var(--c-text-2)", background:"var(--c-track)",
            padding:"2px 8px", borderRadius:4 }}>Metadata Eval v1.0</span>
        ...
        <p style={{ fontSize:13, color:"var(--c-text-2)", marginTop:6, marginBottom:0 }}>
          Quality scores from catalog metadata + paper research. Null = not found in paper.
        </p>
      </div>
